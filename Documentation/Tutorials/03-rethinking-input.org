...
- ~keyboard-event~ to handle refreshing and exitting the console
...
two arguments: the client and the region. For now we'll ignore the region.
...
This function is orthogonal to ~flush-buffer~, because it is expected populate
the buffer in some application-specific manner, while ~flush-buffer~ is used
to "show" the content on the screen. It is time to write a game!
