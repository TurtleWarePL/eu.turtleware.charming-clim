...
We can implement most things with ANSI escape sequences and various
...
And here's how we will use it. ASDF will compile the C file and load
the resulting shared library. To allow interactive programming we'll
use a function ~show-screen~ and call it once per second.
...
some few extensions to receive the pointer events. To understand
better how escape sequences work you may read into the following:
...
shows an important property of the terminal: the first character is
...
Now we'll improve the loop in ~start-display~ to easily modify the
configuration. This step will require restarting the application
because we modify the entry point.
...
There are two characters that have a special meaning. ~ESC~ starts the
escape sequence and ~DEL~ is not a graphical character (despite not
being a control character). We will signify their uniqueness by
...
C1 group), escape sequences, ~delete~ and other alphanumerical
characters. Control sequence and other modifiers like Alt must be
encoded, so we will define a simple class representing a gesture. The
...
Parsing an escape sequence requires more code. The exact sequence
format for special keys varies between terminal emulators. On top of
that some sequences are ambiguous. We are going to focus on the most
popular ones which should be compatible with most emulators. When the
escape character is read and there is no further input, we return the
keyword ~:escape~. Otherwise we try to parse the sequence.
...
defined resolver for a sequence we create an "unknown" gesture, which
may be inspected to learn the reported escape sequence.
...
With all that in place, all what is left is the test code. It will
print characters which are on the terminal, so we can verify if they
are properly recognized. Notice, that we do not clear a whole line
after printing the sequence (only the reminder of it) to avoid
unnecessary flicker. Some key combinations like Alt+F4 may be
...
We'll define a predicate deciding whether a key matches supplied
parameters.
...
Notice that instead of a closure we use the slot ~app~. Function
~user-action~ may be defined from REPL – when C-u is pressed it will
be executed. It may be used for instance to change the
configuration. We still need to add appropriate methods that set the
console on the configuration change.
...
and may be nested to achieve the intersection of the clipping areas.
...
that's the sequence that is documented as a result. We will define a
...
~sigwinch~, but we'll settle on a simpler solution which will be
...
mouse. There are few possible modes (i.e track only mouse clicks), as
well as a few schemes for reporting events. We are interested in
tracking all events reported in the extended scheme (because the
"default" scheme has serious limitations due to byte encoding).
...
Some terminal emulators bind the command "paste primary selection" to
the middle button press action, so don't be surprised when they do.
...
In the second post we'll construct a frame manager with windows and
animations as well as scrolling and other common abstractions. In the
third one we'll go through state-of-the-art algorithms to be able to
render lines, curves, and such. The fourth will extend concepts of the
third one by adding styles and alpha blending. The fifth part will be
a guide about how to write a backend for McCLIM.
...
My main motivations were to make some rendering issues in McCLIM
apparent by growing a pixel into something as big and non-square as a
terminal cell and to have plenty of fun while hacking something
amusing. The second goal is already achieved!

#+CAPTION: WIP frame manager
#+NAME:   fig:frame-manager
[[./static/frame-manager.webm]]

I'd like to thank Jānis Džeriņš, Michał Herda and Christoph Keßler for
offering the help with this text review and providing useful hints.
All mistakes are mine :-). Please don't hesitate to contact me with
questions and remarks.

If you feel like supporting me with my FLOSS contributions you may
become my patron here https://www.patreon.com/jackdaniel_kochmanski.
