...
the control key and (by default) the keypad function keys. The associated data
is a synthesized pointer event.

It is possible for both pointer cursors to be enabled at the same time.
...
- r1, c1, r2, c2 :: surface dimensions in its sink's coordinates
- row0, col0 :: scroll offset
...
Helper functions:
- (scroll-buffer buf row-dx col-dx) :: scrolls the buffer
- (move-buffer buf row-dx col-dx) :: moves the buffer in parent's coordinates
