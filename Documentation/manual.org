...
- input    :: store user gestures
- marker   :: signify input areas
- pointer  :: select an element
...
- cursor-pen,      change-cursor-pen        :: gf
...
flags like ~enabledp~ or ~visiblep~. The console maintains three
...
the control key and a digit. When the pointer is disabled they are not
interpreted. The associated data is a synthesized pointer event. It is
possible for both pointer cursors to be enabled at the same time.
