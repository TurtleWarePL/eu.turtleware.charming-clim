...
its cached value disregarding of the rendering mode. When a cell is
dirty it means that it should be redrawn when flushing the buffer in
the indirect rendering mode.
...
We may easily address this problem by forcing all cells to be
flushed.
...
buffer differently. Report demo has now a constant output which
doesn't change, but we can easily imagine, that it is a buffer with a
text which may change at some point of time, until then the output
remains constant.

| demo      | display     | change source     |
|-----------+-------------+-------------------|
| lambda    | incremental | a new frame (fps) |
| noise     | full redraw | a new frame (fps) |
| animation | incremental | a time slice      |
| report    | full redraw | asynchronous      |

With our rendering modes we can model each behavior, however the frame
manager demo exhibits only one model: a full redraw on a new scene.
This is certainly correct given that we buffer output, however it is
not optimal.

We'll call the full redraw on a new frame an "immediate" mode, while
all others will fall into a "retained" mode category. Notice, that in
the former case when the console is in a buffered mode, having the
noise demo frame in a direct rendering mode is conceptually correct.

In order to prevent overexposing of the immediate windows we need to
introduce a concept of a "damaged region", so only the damaged part of
a retained window is redrawn without re-displaying whole content.
