...
This practice greatly improves quality of life of the person who works
...
when in fact we are bound by the I/O. We will do some exploratory
...
terminal, number of characters written per frame. Finally, two
compound metrics: average number of writes per single terminal cell
and the write velocity (total number of characters per second).

We will display all in the modeline. Common Lisp have internal time
which has usually the unit being 1/1000s. This precision is not good
enough. For instance if we draw 2000fps the time difference will be
less than the internal time unit. Instead we'll count number of frames
which we were able to render during a period of one second. To measure
...
#+NAME:   fig:modeline
[[./static/02-modeline.webm]]

Current demos are not representative, because they do not fill all
cells in the terminal. For that we'll use a full screen noise demo and
turn off the lambda demo. It fills whole terminal except the last row
where we display the modeline. To reduce the noise (ha ha!), we'll
skip the window decorations and setting the "activity" color.
...
You may need to type ~C-e~ and restart display from a debugger to
...
with ~C-r~. For 25x80 terminal it is around 23fps with 125 writes per
single cell and around 5M characters per second.

#+CAPTION: Counting FPS (full screen)
#+NAME:   fig:crap-fps
[[./static/02-crap-fps.webm]]

One obvious optimization is to call ~finish-output~ after each frame
rendered, not after each sequence put on the terminal. We'll abstract
flushing the buffer with a generic function ~flush-buffer~ which will
be a part of the virtual buffer protocol. It will be accompanied with
the new ~ctl~ operation called ~:fls~.
...
receive the response synchronously (like in ~get-cursor-position~).
...
nice. For 25x80 terminal it is around 53fps with 125 writes per single
cell and around 13.5M characters per second.

Now let's examine our CPU and I/O bounds. First recompile ~out~ and
~ctl~ macros to do nothing, compile-and-load ~example.lisp~ file and
refresh the display with ~C-r~. After that probe the fps from a repl.
...
on the screen). ~FPS (i/o)~ column gives us the best score we can
...
number will be beneficial. Notice, that we do little too much since
...
The macro ~out~ also sets row, column, foreground and background
colors, and ~:after~ auxiliary methods configure the terminal. In
other words for each character we:
...
As expected, number of writes per cell drops threefold. ~WPC~ column
is now constant (for a full screen applications which write each cell)
and amounts 40ch/cell. Fix the ~out~ macro so it doesn't change the
slot in console - it is unnecessary anymore.
...
of ~put-cell~.  Let's take one step at a time and move the logic from
the macro ~out~ to the method ~put-cell~.
...
random color - it will be less favorable if the output cell will be
also random (like in the lambda demo).

* Rendering modes

Let's modify the noise demo to accept a sequence of colors from which
the foreground color is drawed by lot. The class frame will have one
...
we can see a flicker! Not only that, lambda demo which is drawn last,
and rightfully should be on top, is obscured by the red noise demo.

#+CAPTION: Flickering windows
#+NAME:   fig:flicker
[[./static/02-flicker.webm]]

The flicker is because of how we draw things. We put each cell
immedietely, so first we draw the first window, then on top of it the
second window and then on top of it the third window. After that we
repeat the process. That means that if we have two intersecting
windows, then for part of the time it will have a content of the first
window and for the rest of a time a content of the second one.

The issue with the lambda demo not being on top is slightly different.
In this demo we draw only one cell per frame, so only one cell may be
drawn on top of the other window, and then the noise demo redraws a
full window.

Another problem which is not visible is a performance penalty. If we
are bound by I/O, then drawing multiple times the same cell is very
suboptimal. Ideally we'd modify each cell only once per frame.
...
virtual buffer to redraw damaged parts of a terminal.

Direct rendering is useful in some applications, so we'll retain this
functionality and allow switching rendering mode for each buffer with
the ~ctl~ interface. Three modes will be defined: a direct rendering,
an indirect rendering and a write-through rendering. The last one will
combine the two: it will put the cell on the screen immedietely but it
will also save its content in a buffer. We'll add three new functions
to the virtual buffer protocol.
...
that is either putting it directly on a screen or saving it in the
internal array (or both). The accessor ~rnd~ is used to read and write
the buffer rendering mode. The macro ~out~ calls now the function
~set-cell~ and the macro ~ctl~ has a new option ~:rnd~.
...
the array stored in a buffer's slot ~data~. We'll also add a slot for
the rendering mode.
...
The function translates the terminal index (starting with [1, 1]) to
...
The array with data has initially dimensions ~(0 0), so we need to
update them when the console dimensions change.
...
function is responsible for updating the row and the column variables
when iterating over the string, so they indicate the correct cell.
Function "wraps", that if we go beyond the last row, we'll start from
the first row (similarily for columns).
...
its cached. When a cell is dirty it means that it should be redrawn
when flushing the buffer in the indirect rendering mode.
...
#+NAME:   fig:buffering
[[./static/02-buffering.webm]]

* Surfaces

We have two problems with the lambda application: the demo can't be
moved (because it starts drawing from ~[1,1]~) and that it is obscured
by a noise demo frame due to infrequent writes. We will now detach the
notion of the application buffer and the console buffer. Our job would
be much easier if we had [[https://turtleware.eu/posts/Conformal-array-displacement.html][conformally displaced arrays]] at our disposal
- a multi-dimensional fill pointer and displacement offset would allow
us to map coordinates transparently. That said we can easily abstract
all that away, because we do not expose naked arrays in the API.
...
#+NAME: fig:no-surface
[[./static/02-no-surface.webm]]

Each application will be rendered on a "surface", that is a virtual
buffer which is displaced onto the console. Surface's internal buffer
starts from coordinates ~[1,1]~ like the console, and then when we
call ~put-cell~, the coordinates are transformed, and ~set-cell~ is
called on the console. The way surfaces are defined means that they
may be stacked (that is the external buffer of a surface may be a
virtual buffer which is another surface or something else). Add a new
file ~surface.lisp~ to the project.
...
they always render starting from ~[1,1]~. The function ~lambda-demo~
doesn't need changes, but the function ~noise-demo~ does, because it
started drawing from ~(r1 frame)~. Lambda demo size is known while for
the noise demo it is inferred from the surface displacement.
...
Lambda is now properly offset, but the noise demo is still
overexposed.

#+CAPTION: Lambda problems
#+NAME: fig:surface
[[./static/02-surface.webm]]

We may easily address that by forcing all cells to be flushed. Later
on we will tackle this problem from a different angle.
...
#+CAPTION: Lambda problems
#+NAME: fig:surface
[[./static/02-surface-flush.webm]]

The last missing functionality is scrolling. The lambda demo does not
fit in its window. We'll introduce two slots in the ~surface~ which
will represent the offset for the top-left corner of the buffer. For
instance when the offset row is 3, then the third row of the buffer
will be shown as the first row in the window. We only need to modify
the function ~put-cell~ to account for that.
...
We'll add now new key bindings in ~handle-event~ to scroll and move
the window. This way we will gain some intuition of how it should
work. When rendering decorations we will indicate that some of the
output is not visible by the character ~#\&~. To avoid glitches we
will also clear a whole screen in the function ~display-screen~ and
...
[[./static/02-surface-scroll.webm]]

While experimenting with the window you may notice some inconsistency:
scrolling moves the content in the opposite direction than moving the
window (if we use the same arrow key). This discrepancy may be
described with an analogy of a cursor: when you scroll right, you move
an invisible cursor beyond the right edge, so the content is moved
left to reveal what is under the "cursor". Alternative strategy, where
pressing "right" moves the content to the right, could be described in
terms of a touchscreen: you hold part of the screen and move it to the
right, so the content moves along your finger. To signify a difference
we'll talk about the "cursor scrolling" and the "finger scrolling".

The last step is to ensure, that we don't scroll too much. Content
scrolling should stop if we reach the maximum. What is considered the
maximum depends on whether the window is bigger or smaller then the
...
we can (so when we for instance cursor-scroll 1000 to the left and the
line has only 100 characters, we'll end at the line beginning).
...
array. It is a matter of specifying correct initargs. We'll add a hack
because our frame manager currently assumes that the surface is a
...
Let's introduce a few more examples to have a few specimen we could
talk about. The animation demo shows a square which bounces from the
left to the right edge, and the report demo shows lines of text.
...
#+CAPTION: New demos
#+NAME:   fig:new-demos
[[./static/02-all-demos.webm]]

When we look at these demos we can recognize, that each uses the
buffer differently. The old demos "lambda" and "noise" outputs change
synchronously when a new frame is drawn. The new demos change based on
asynchronous events - for the "animation" demo that is a time slice,
for the "report" demo it is (hypothetically) a buffer contents change.
...
term of buffers it could be explained like this:

- displaying :: drawing on the buffer in ~:buf~ mode
...
In the retained rendering mode the separation of concepts is
...
of full blown objects which have their own repaint methods. In CLIM a
display list is called the ~output-record-history~, and elements of
said list are ~output-records~. Compound output records may contain
more (inner) output records so objects in such display list form a
tree with z-ordering.

We will explore the topic of retained display and display lists
further in another post which will introduce yet another layer of
abstraction.

* Conclusions

I hope you've liked this post as much as I've enjoyed working on it.
It has grown considerably longer than I have anticipated, so I've
decided to postpone the discussion of display lists and damage regions
for later time. The next post in this serie will cover input
processing.

If you like this kind of work, you may [[https://www.youtube.com/watch?v=RziVt_1Ac7g][toss a coin to your Lisper]] by
[[https://turtleware.eu/donate.html][supporting]] me financially.
