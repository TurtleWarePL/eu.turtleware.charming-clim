...
(defun invoke-with-console (cont console-class &rest console-args)
  (loop (restart-case
            (let ((*terminal* (getf console-args :ios))
                  (*console* (apply #'make-instance console-class console-args)))
...
(defmacro with-console ((&rest args
  ...
  `(invoke-with-console (lambda () ,@body) ,console-class ,@args))
      ...
      (if (and cell (or force (dirty-p cell)))
          (let ((chr (chr cell))
            ...
            (unless (= bgc last-bgc)
              ...
              (setf last-bgc bgc))
            (ax:when-let ((diff (text-style-diff txt last-txt)))
              (set-text-style diff)
              (setf last-txt txt))
            (when (plusp gap)
              (cursor-right gap)
...
(defmethod handle-event :before ((client console) (event terminal-resize-event))
  (let ((rows (rows event))
    ...
    (let* ((vptr (vrt client))
           (row (ax:clamp (row vptr) 1 rows))
           (col (ax:clamp (col vptr) 1 cols)))
...
(defun handle-vptr (client vptr key)
  ...
  (let ((event (cursor-data vptr))
    ...
    (labels ((set-position (row col)
               (let ((row (ax:clamp row 1 (rows client)))
                     (col (ax:clamp col 1 (cols client))))
                 (setf (row event) row
                       (col event) col)))
             (click (btn)
               (setf (btn event) btn)
