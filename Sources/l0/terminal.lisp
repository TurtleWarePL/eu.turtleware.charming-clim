...
;;; KLUDGE there are in fact two delete keys: one on the keypad and one
;;; generated by some terminals when pressing the backspace. The latter is
;;; returned as :RUBOUT.
(eval-when (:compile-toplevel :load-toplevel :execute)
  (defconstant +delete+ (code-char #x7f)
    "The DEL character (#\Rubout), last in the ASCII table.")
  (defconstant +backspace+ (code-char #x08)
    "The backspace character (C-h).")
...
(defun maybe-combo (key num2)
  (let ((mod (if (null num2)
    ...
    (unless (or (eq +alt-mod+ +alt-mod*+)
      ...
      (decf mod +alt-mod*+)
      (incf mod +alt-mod+))
    (ax:if-let ((ctrl (and (characterp key) (controlp key))))
      (prog1 ctrl
        (setf (mods ctrl)
...
(defun escapep (ch)
  ...
  (ax:if-let ((next-ch (read-char-no-hang *terminal*)))
...
(defun deletep (ch)
  (cond ((char= ch +delete+)
         (make-instance 'keyboard-event :key :rubout :mods 0))
        ((char= ch +backspace+)
         (make-instance 'keyboard-event :key :backspace :mods 0))))
                    ...
                    (let* ((test (car clause))
                           (real-test
                             (if (member test '(otherwise t))
                                 t
                                 `(keyp ,event ,@(ax:ensure-list test)))))
